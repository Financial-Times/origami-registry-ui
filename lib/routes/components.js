	...
	app.get('/components/:componentId/readme', cacheForFiveMinutes, findComponent, findBrand, async (request, response, next) => {
		...
		try {
			...
			if (listHtml.indexOf('usage') || listHtml.indexOf('javascript') || listHtml.indexOf('sass')) {
				nav.addClass('registry__sidebar-list');
				nav.attr('data-o-component', 'o-component-listing');
				nav.prepend($('<li><span class="registry__sidebar-list-title" data-o-component-category="readme">Readme</span></li>'));
				$('li', nav).addClass('registry__sidebar-list-item').each((index, element) => {
					$(element).attr('data-o-component-name', $('> a', element).first().text());
					$(element).attr('data-o-component-keywords', `["${$('> a', element).first().text()}"]`);
				});
				...
				$('h2, h3').each((index, element) => {
					...
					const lastAddedLevelTwo = navItems[navItems.length - 1];
					if (isLevelTwo) {
						navItems.push({ title, link });
					} else if (lastAddedLevelTwo) {
						lastAddedLevelTwo.items = lastAddedLevelTwo.items || [];
						lastAddedLevelTwo.items.push({title, link});
					}
				});
			}
