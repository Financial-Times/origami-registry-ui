...
const SyntaxHighlight = require('../../bower_components/o-syntax-highlight/src/js/syntax-highlight.js');
		...
		try {
			...
			try {
				await Promise.all(demos.filter(demo => demo.display.html).map(async demo => {
					...
					const highlighter = new SyntaxHighlight(demo.display.plainHTML, {language: 'html'});
					console.log({highlighter});
					demo.display.highlightedHTML = highlighter.tokenise();
				}));
			} catch (error) {
			}
