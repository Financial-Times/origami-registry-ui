...
module.exports = app => {
	...
	const findBrand = (request, responce, next) => {
		request.brand = (request.query.brand) ? request.query.brand : 'master';
		next();
	}

	const findComponent = async (request, responce, next) => {
	...
	app.get('/components/:componentId', cacheForFiveMinutes, findComponent, findBrand, async (request, response, next) => {
		...
		const componentName = request.component.name;
		const componentVersion = request.component.version;
