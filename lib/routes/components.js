...
module.exports = app => {
	...
	app.get('/components', cacheForFiveMinutes, express.urlencoded({extended: false}), async (request, response, next) => {
		try {
			...
			repos = repoListing.markVisibilityByBrand(repos, filter.brand);
	...
	app.get('/components/:componentId', cacheForFiveMinutes, async (request, response, next) => {
		...
		try {
			...
			//if a brand has been requetsed, set it to the component to pass on to the component page
			if (requestedBrand) {
			...
			if (component.type === 'module' && component.resources.demos) {
				...
				// if a brand has been requested, load the branded demos
				if (requestedBrand) {
					try {
						await Promise.all(demos.filter(demo => demo.supportingUrls.live).map(async demo => {
							demo.supportingUrls.live += `?brand=${requestedBrand}`;
						}));
					} catch (error) {}
				}
