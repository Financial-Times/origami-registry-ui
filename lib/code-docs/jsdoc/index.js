...
class JsDoc {
     ...
     * @param {Array} allSupportedDoclets - An array of all supported json doclets.
     * @returns {Object} Formatted node.
     */
    static formatDoclet(doclet, allSupportedDoclets) {
        switch (doclet.kind) {
            case 'class':
                return new ClassNode(doclet, allSupportedDoclets);
                break;
            case 'function':
                return new FunctionNode(doclet, allSupportedDoclets);
                break;
            case 'constant':
            case 'member':
            ...
            case 'mixin':
                ...
                break;
            case 'typedef':
                return new TypedefNode(doclet, allSupportedDoclets);
                break;
            case 'module':
                return new ModuleNode(doclet, allSupportedDoclets);
                break;
            default:
    ...
    getNodes() {
        if (!this._formattedDoclets) {
            ...
            this._formattedDoclets = supportedDoclets.map((doclet) => {
                return JsDoc.formatDoclet(doclet, supportedDoclets);
            });
        }
