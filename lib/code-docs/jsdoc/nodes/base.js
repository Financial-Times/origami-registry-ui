...
class BaseNode {
    ...
    /**
     * @param {Object} doclet
     * @param {BaseNode} target - The node to add properties to.
     */
    addTypes(doclet, target = this) {
        target.types = doclet.type && Array.isArray(doclet.type.names) ? doclet.type.names : [];
    }
     ...
     * @param {BaseNode} target - The node to add properties to.
     */
    addProperties(doclet, target = this) {
        target.properties = target.properties || [];
        if (Array.isArray(doclet.properties)) {
            doclet.properties.forEach((property) => target.properties.push({
                'name': property.name,
                'types': property.type && property.type.names ? property.type.names : [],
                'description': target.replaceLinks(property.description) || ''
            }));
        }
    }
     ...
     * Handles parameter for standard JSDoc jsdon doclets,
     * and custom parameter types added by the registry.
     * @param {Object} doclet
     * @param {BaseNode} target - The node to add parameters to.
     */
    ...
    /**
     * @param {Object} doclet
     * @param {BaseNode} target - The node to return values to.
     */
    addReturns(doclet, target = this) {
        if (doclet.returns) {
            target.returns = {
                'types': doclet.returns[0] && doclet.returns[0].type && doclet.returns[0].type.names ? doclet.returns[0].type.names : [], // return can be of multiple types
