...
class BaseNode {
    ...
    addParameters(doclet, target = this, allSupportedDoclets = []) {
        target.parameters = target.parameters || [];
        if (Array.isArray(doclet.params)) {
            doclet.params.forEach((param) => target.parameters.push({
                'name': param.name,
                'types': param.type && param.type.names ? param.type.names.map((type) => {
                    const typeDoclet = allSupportedDoclets.find(doclet => doclet.longname === type);
                    return {
                        name: type,
                        longname: typeDoclet ? typeDoclet.longname : null
                    };
                }) : [], // param can accept multiple types
